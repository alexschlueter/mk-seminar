...
Für jede Teilmenge $J\subset\{1,\dotsc,n\}$ definiere $f_J:\Omega\to\RR$ durch
 ...
 \begin{dproof}
  ...
  \begin{align*}
   \sprod{f_I,f_J}_\pi&=\sum_{x\in\Omega}f_I(x)f_J(x)\pi(x) \\
   &=2^{-n}\sum_{(x_1,\dotsc,\cancel{x_k},\dotsc,x_n)}\sum_{x_k\in\{-1,1\}}x_k\cdot f_{I\setminus\{k\}}(x)f_J(x) \\
   &=0\, ,
  \end{align*}
...
\begin{exmpbk}[Träge Irrfahrt auf dem $2n$\=/Zykel]
 ...
 \end{equation}
 Für $0\leq j<2n$ definiere $f_j(x)\coloneqq x\cdot\omega^j$. Dann gilt für alle $k\in\{0,\dotsc,2n-1\}$
 \begin{align*}
 ...
 \end{align*}
 Damit haben wir $2n$ verschiedene Eigenwerte gefunden und der zweitgrößte ist
 \begin{equation*}
  \lambda_2=\frac{1+\cos(\pi/n)}{2}\, =1-\frac{\pi^2}{4n^2}+\mathcal{O}(n^{-4}).
 \end{equation*}
